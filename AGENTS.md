...
- Provider query starts CC with `tools: []`. Every tool arrives over MCP. Any
  tool_use naming a builtin is a hallucination — must not reach pi. Web reach is
  a distil CLI capability (`distil web search|fetch`), never a provider builtin.
