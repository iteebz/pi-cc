#!/usr/bin/env bash
# Multi-turn integration tests for pi-cc provider.
...
run_json "single-turn: multiple sequential tool calls" \
  '([.[] | select(.type == "message_update") | .assistantMessageEvent | select(.type == "toolcall_end")] | length) >= 2 and
   ([.[] | select(.type == "message_update") | .assistantMessageEvent | select(.type == "text_end") | .content] | join(" ") | test("pi-cc")) and
...
# "pi-cc"), turn 2 reads LICENSE (has "MIT License").  If stale
