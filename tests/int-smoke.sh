...
ALT_PROVIDER=$(require_env CC_BRIDGE_TESTING_ALT_PROVIDER)
ALT_MODEL=$(require_env CC_BRIDGE_TESTING_ALT_MODEL)
...
run "provider: --provider + --model resolve to the bridge" \
  bash -c "pi --no-session -ne -e '$DIR' --provider cc --model claude-haiku-4-5 --mode json \
    -p \"Reply with just the word 'yes'\" 2>/dev/null \
    | jq -r 'select(.message.role==\"assistant\") | .message.provider + \"/\" + .message.model' \
    | sort -u | grep -x 'cc/claude-haiku-4-5'"

run "provider: model list includes provider" \
  bash -c "pi --no-session -ne -e '$DIR' --list-models 2>&1 | grep cc"
...
run "system prompt: --append-system-prompt reaches Claude" \
  bash -c "pi --no-session -ne -e '$DIR' --model 'cc/claude-haiku-4-5' \
